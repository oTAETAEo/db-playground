# 고빈도 백그라운드 금융 연산 환경에서의 DB 락을 통한 지갑 잔액 차감 동시성 제어 및 자산 정합성 실험 리포트
...
- 낙관적 락을 사용하는 경우 동시성 환경에서의 주문 내역과 실제 차감된 돈의 정합성이 맞지만 총 10000원의 금액 차감은 나타나지 않았다
```text
낙관적 락 정합성 테스트 결과
생성된 총 주문 내역 수: 15건
...
- 낙관적 락을 사용할때는 재시도 로직이 필수인것 같고, 이런 많은 트래픽이 발생하는 부분에서는 적합하지 않다고 생각한다.
...
- ```text
    ...
    ```
  - 이 문제는 비관적 락 테스트를 진행할 때 마주진 예외이다, 이 예외는 `@Lock(LockModeType.PESSIMISTIC_WRITE)` 로 `Repository` 메서드에 락을 걸게 되는데 <br> 이렇게 락이 걸린 메서드가 트랜젝션이 실행되지 않는 곳에서 호출했을 경우 나타나가 된다
